...
登録した服を写真ファーストのグリッドで一覧化します。件数付きチップによるカテゴリ絞り込み、シーズン（SS / AW）絞り込み、名前・ブランド・メモのキーワード検索ができます。

![ワードローブ一覧](.github/screenshots/wardrobe.png)

### コーデ記録

「その日に実際に着たコーデ」を記録します。保存済みスタイリングから 1 回の選択で起こすこともできます。記録は日付を添えたタイムラインで新しい順に振り返ります。

![コーデ記録](.github/screenshots/outfits.png)

### ログイン

クローゼットの扉が開いてアプリが始まる、線画の演出を採用しています。この「細い青線の線画」はスタイリングボードのクローゼットにも通底するデザイン言語です。
