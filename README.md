...
このアプリの中核となる機能です。服好きの誰もが経験する、クローゼットから大量の服をカーペットの上に並べ、組み合わせを試す自分用ファッションショーをアプリで実現可能にしました。引き出しからカーペットへアイテムをドラッグし、組み合わせを試作するホワイトボード式キャンバスを用意しました。そこへ位置・重なり順・大きさごとに保存され、開き直すと同じ見た目で再現されます。これで外出中や深夜で物音を立てられない時にもスタイリングを楽しむことができます。
