...
**お気に入りスタイリングを、蓄積して使い回すためのワードローブ基盤。**
...
**本番環境: https://wardrobase.vercel.app** — ログイン画面の「デモを見る」から、登録なしで閲覧専用デモを試せます。

## スクリーンショット

### スタイリングボード

このアプリの中核となる機能です。服好きの誰もが経験する、クローゼットから大量の服をカーペットの上に並べ、組み合わせを試す自分用ファッションショーをアプリで実現可能にしました。引き出しからカーペットへアイテムをドラッグし、組み合わせを試作するホワイトボード式キャンバスを用意しました。配置は位置・重なり順・大きさごと保存され、開き直すと同じ見た目で再現されます。これで外出中や深夜で物音を立てられない時にもスタイリングを楽しむことができます。

![スタイリングボード](.github/screenshots/board.png)

### ワードローブ

登録した服をグリッドで一覧化します。カテゴリ・色・季節での絞り込みと、ブランド・メモのキーワード検索ができます。

![ワードローブ一覧](.github/screenshots/wardrobe.png)

### コーデ記録

「その日に実際に着たコーデ」を記録します。保存済みスタイリングから 1 回の選択で起こすこともできます。記録はルックブック（本）として振り返ります。広い画面では線画の本にポラロイド写真が貼られ、ページをめくって過去を辿れます。スマホではリング綴じのノートになり、日付スタンプとポラロイドの散らし貼りで 1 日分を綴ります。

![コーデ記録](.github/screenshots/outfits.png)

### ログイン

クローゼットの扉が開いてアプリが始まる、線画の演出を採用しています。この「細い青線の線画」はワードローブのたんす・ルックブックの本・リングノートにも通底するデザイン言語です。
